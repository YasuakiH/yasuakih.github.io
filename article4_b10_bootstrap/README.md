...
これに対し、同等性検証では考え方を逆転させ、「差がゼロである」ことではなく、「差が許容できる範囲に収まっている確信 (信頼区間) があること」を基準にする。
...
本シミュレーションで採用した二重片側検定 (TOST: Two One-Sided Tests) と呼ばれる同等性検証の手法では、あらかじめ「この範囲に収まれば同等」という許容範囲を設定する (本シミュレーターでは旧型B10比 −10% ～ +50%) 。算出した信頼区間をこの許容範囲と照合して、次の3通りで判定する。
