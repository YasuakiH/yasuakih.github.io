...
<div align="center">
  <figure>
    ...
    <br/>
    <figcaption>シミュレーション結果<a href="img/fleet_reliability_simulator(12, 4.0).png" target="_balnk"> [別ウィンドウで開く]</a></figcaption>
  </figure>
</div>
...
- **⑧ 意思決定サチュレーション**<br/>
   ...
   累積故障率 F < 30% のデータポイントを用いたが、特に稼働開始まもない初期段階ではN数が少ないため、参考用として全データを用いてプロット、色で区別した。
    ...
    新型機の12ヶ月目では、B10推定値 (青; 折れ線) が上昇傾向にあり、サチっていない。7ヶ月目以降、B10推定値の信頼区間 (青色バンド) が真値 (青点線) を含んでいるが、その幅に旧型機の実績ライフ (燈点線) をまたぐことから、より多くのサンプルが必要と言える。
...
* CENSORING_FACTOR: 予防保守のタイミングを決定する係数。設計ライフ(B10)に対する倍率(例: 4.0)。この値を下げるほど故障は減るが、摩耗故障データが取れなくなり予測分布が広がる。
