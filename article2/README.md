...
## 部品ライフの推定方法
保守サービスにおける課題の一つに、サービス提供に要するコストと、顧客が受け取るサービス品質との両立がある。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を及ぼすと仮定した場合、サービス提供者の関心は、サービス品質に直結するパラメータ「定期交換部品の交換時期」を最適にすることに向けられる。このパラメータ (交換時期) を変化させたとき、サービス提供に要するコストへの影響と、顧客に生じるリスクへの影響を予測することが求められる。印刷機に次々と入力される印刷ジョブに対する部品の生存-故障の推定と、保守サービスによる部品の交換をシミュレートすることで、部品の交換数 (コスト) と印刷機の停止時間 (ダウンタイム) を予測することができる。

### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)に基づき、機械の使われ方から部品ライフを推定する故障モデルを実装した。応力-強度モデルは応力 S と強度 R が分かっている場合に、故障の確率 F を予測するモデルである。<!-- 応力 S × 強度 R → 故障確率 F という関係を導く。--> 強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。応力 S > 強度 R の場合に故障が起こると考え、故障確率 F (failure probability) が与えられる。
