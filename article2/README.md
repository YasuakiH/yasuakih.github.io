...
### 部品ライフ推定
本スタディでは、故障現象を理解しやすくするため、印刷用紙の搬送系の部品 (例: ローラー、ベルト) で考える。次のように段階的に対応した。

<dl>
  <dt>①未知パラメータから部品負荷の推定</dt>
  <dd><a href="../article1/#未知パラメータと推定">未知パラメータ</a>によって表された印刷機の使われ方を元に、部品にかかる負荷を推定する。部品にかかる負荷は、印刷ジョブによって運動する用紙の搬送距離として見積もった。
  </dd>

  <dt>②特定の印刷機における部品ライフの推定</dt>
  <dd>印刷機全体の母集団における部品の強度 R を<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似し、<a href="https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0">生存関数</a> (SF) を推定する。無作為に作成した印刷ジョブによって部品にかかる負荷を累積し、各々の時点における条件付き生存率を計算し、一様乱数との比較によって部品が生存するか故障するかを判定した。故障した部品、あるいは累積ライフが管理目標を超えた部品は新しい部品で置き換え、ライフと理由を交換履歴に記録する。
  </dd>
...
### 詳細な構造
#### ①未知パラメータから部品負荷の推定
印刷機の使われ方を推定した<a href="../article1/#未知パラメータ">未知パラメータ</a>をもとに、部品に負荷をかけるための印刷ジョブを生成する。前述のように搬送系の部品に着目することから、部品にかかる負荷は、印刷ジョブによって運動する用紙の搬送距離として見積もった。未知パラメータから「印刷ジョブ長」と「用紙サイズ」を無作為にサンプリングし、用紙長さと印刷ジョブ長の積を搬送距離 ΔT とした。
...
#### ②特定の印刷機における部品ライフの推定
印刷機全体の母集団における部品の強度 R をワイブル分布で近似し、生存関数</a> (survival function, SF) を推定する。無作為に多数作成した印刷ジョブによって部品にかかる負荷 ΔT を累積し、各々の時点における条件付き生存率を計算し、一様乱数との比較によって部品が生存するか故障するかを判定した。故障した部品、あるいは累積ライフが管理目標を超えた部品は新しい部品で置き換え、ライフと理由を交換履歴に記録する。

>  条件付き生存率<br/>
>  部品の故障は条件付き生存率 (CS) で考えることができる。[生存関数](https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0) SF は経年に伴って生き残った部品の割合である。
