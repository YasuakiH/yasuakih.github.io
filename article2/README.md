...
## シミュレーションの設計
### 部品ライフ推定システム
これは予防保守による交換時期による信頼性とコストへの影響を評価するためのソフトウェアである。次に構成を説明する。
...
<dl>
  ...
  <dd>前述の故障モデルとシミュレーションモデルを含む。故障モデルの部品強度 R は<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似した。ワイブル分布は修理しない部品ライフのモデル化によく使われ、2つの変数 (形状パラメータ、尺度パラメータ) のみで特徴付けられることから運用にも優れる。これらのパラメータはコマンドラインで与えた。もし保守サービスで交換された部品交換の実績があれば統計的に算出することもできる。

シミュレーションが実行されると、機能①から入力された印刷ジョブによって部品にかかる負荷を累積し、ワイブル分布にもとづく生存関数 (SF) と一様乱数を用いて生存-故障判定を行い故障発生をシミュレートし、部品ライフを推定した。

これと同時に、エンジニアによる予防保守と障害修理をシミュレートし、故障が見込まれるあるいは故障した部品を刷新した。
交換された部品のライフ、交換理由、交換に伴う印刷機の停止時間は交換履歴として集められ、後続の③と④に出力する。
