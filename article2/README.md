...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルで、応力 S × 強度 R → 故障確率 F という関係が導かれる。本スタディの場合、部品の強度 R は保守サービスを通じて収集することができるが、部品にかかる応力 S は、顧客先で稼働している印刷機で直接に測定することはできない。したがって、ここで採るアプローチは応力-強度モデルに依らず、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。
...
<dl>
  ...
  <dd>印刷機全体の母集団における部品ライフを部品強度 R と見なし、これを<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似して<a href="https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0">生存関数</a> (SF) を推定する。無作為に作成した印刷ジョブによって部品にかかる負荷を累積し、各々の時点における条件付き生存率をもとに部品が生存するか故障するかを決定する。部品が故障したり、累積ライフが管理目標を超過したら交換する。
  </dd>

  <dt>③応力-強度モデル図作成</dt>
  <dd>_このステップは必須ではない。_ 交換履歴をもとに、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求める。故障確率 F を強度 R で除し、応力 S を推定する (参考用)。
  </dd>
