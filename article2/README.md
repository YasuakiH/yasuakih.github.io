...
<dl>
  ...
  <dd>未知パラメータによって表された印刷機の使われ方をもとに、部品にかかる負荷を生成する。搬送系の部品の負荷は印刷ジョブによって移動する用紙の搬送距離である。
  </dd>

  <dt>②特定の印刷機における部品ライフの推定</dt>
  <dd>部品強度 R は、保守サービスで交換された部品ライフから統計的に算出できる。ここでは部品強度 R を<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似する。ワイブル分布は修理しない部品ライフによく使われ、2つの変数 (形状パラメータ、尺度パラメータ) で特徴付けられることから運用にも優れる。無作為に作成された印刷ジョブによって部品にかかる負荷を累積し、ワイブル分布をもとづく生存関数 (SF) を用いて部品ライフを推定する。
  </dd>
