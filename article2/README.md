...
<blockquote>
<div align="center"><figure><img src="img/応力強度モデルチャート(AC=L,PL=M)(iter=20)/応力-強度干渉グラフ(AC=L,PL=M)(0.80).png"/><br/><figcaption>応力-強度干渉モデル図を示す (AC=L,PL=M)(管理目標係数0.80)<br/>(上段) ワイブル分布で近似した部品の強度 R の確率密度(青線)。広がった裾野は低いライフで故障が起こる可能性を示している。(中段) ヒストグラムは、障害修理で交換された部品(桃色)と、予防保守で交換された部品のライフ分布(水色)で、印刷機の「使われ方」を表すものである。緑色の実線は交換時期の管理目標、緑色の点線は部品ライフの算術平均値を示す。これをもとに対数正規分布で近似した応力 S (赤線) は、強度 R とは異なる分布わかる。(下段) で応力 S が強度 R を上回る領域が強調され、その面積が故障確率に対応している。
