...
## 結論
オンデマンド印刷機の定期交換部品の最適な交換時期を推定するため、応力-強度干渉モデルに基づく故障モデルと、離散事象シミュレーションモデルからなるコンピュータ・シミュレーションを構築した。お客様に固有の未知パラメータを利用して印刷ジョブ長と用紙サイズで特徴付けた印刷ジョブを生成し、用紙の搬送距離を部品にかかる負荷とした。部品の強度に基づく生存関数と累積負荷から故障交換を、また交換時期に基づく予防保守で打ち切り交換をそれぞれシミュレートし、これらの部品ライフの分布からお客様に固有の応力を推定した。シミュレーションの結果、交換時期の変更が応力に影響を及ぼし、交換部品数 (コスト) と機械停止時間 (ダウンタイム) を変化させることが分かった。また交換時期を変化させて改善効果を推定する応用ができることも確認した。課題として人的コストを含めたより包括的なシミュレーション実施がある。
