...
### 結果
#### 交換部品数
交換時期を遅らせることによるトータルの部品交換数は低下傾向であった。交換理由の別では、「予防保守」(水色) は減り続け、一方「障害修理」(桃色) は増え続けた。本シミュレーションでは故障の発生は確率的であることから、経年が少ない場合でも故障は起こり障害修理が発生するため、交換時期を早めても初期故障を防ぎきれないことが示した。
...
#### 停止時間
交換理由の別では、交換時期を遅らせることで「予防保守」(水色) は減少傾向、「障害修理」(桃色) は増加傾向が見られた。本シミュレーションの停止時間は、(計画的な)予防保守が30分に対し、(突発的な)障害修理は60-90分とペナルティを加えた。そのため交換部品数の減少は停止時間の増加で相殺され、予防保守と障害修理を合計した機械の停止時間は一定 (標準誤差の範囲内) であった。
