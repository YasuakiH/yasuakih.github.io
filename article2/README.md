...
### 部品ライフ推定システム
印刷機における定期交換部品の故障はさまざまあるが、本スタディでは理解しやすい用紙の搬送系の部品 (例: ローラー、ベルト) を取り上げる。そのために、次の5つの機能ブロックを組み合わせて部品ライフを推定するシステムを構築し、保守サービスの管理目標値を信頼性・コストの両面から評価する。

<dl>
  <dt>①未知パラメータから部品負荷の推定</dt>
  <dd><a href="../article1/#未知パラメータと推定">未知パラメータ</a>によって表された印刷機の使われ方をもとに、部品にかかる負荷を生成する。搬送系の部品では、印刷ジョブによって移動する用紙の搬送距離が一例である。
  </dd>
...
### 印刷機の保守モデル
デジタル印刷機に関連するさまざまな事象を表現し、定期交換部品の経年と故障、エンジニアによる保守サービスをコンピューターでシミュレートするために「保守モデル」を作成した。保守モデルは前述した故障モデルを実行するために、リソース、プロセス、データといったオブジェクトを含む。
