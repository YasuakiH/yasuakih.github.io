...
### 保守モデル
デジタル印刷機で行われる、定期交換部品の経年消耗と、エンジニアによる保守サービスをコンピューター・シミュレーションするために「保守モデル」を作成した。保守モデルにはリソース、プロセス、データなどが含まれ、これらが仮想的な環境でそれぞれの役割を果たすことで前述した故障モデルを実行する。
...
加えて、印刷機に入力される印刷ジョブの生成、前述した故障モデルに基づく故障の発生、計画的に行われる予防保守、故障した部品を交換する障害修理など、各種イベントが非同期的に発生する。

こうした一連の処理を記述する保守モデルを作成した。
