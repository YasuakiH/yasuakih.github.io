...
## 部品ライフの推定方法
保守サービスにおける課題は、サービス提供によるコストと、顧客が受け取るサービス品質の両立である。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を与える場合に関心があるのは、サービス品質に直結するパラメータである定期交換部品の交換時期を変化させたとき、サービス提供に生じるコストへの影響と、顧客に生じるリスクへの影響である。一つ一つの部品ライフを推定して、部品の交換数 (コスト) と、印刷機の停止時間 (ダウンタイム) を子細に検討した。
