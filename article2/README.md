...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルで、応力 S × 強度 R → 故障確率 F という関係が導かれる。本スタディの場合、部品の強度 R (resistance) は保守サービスを通じて収集することができるが、部品にかかる応力 S (stress) は、顧客先で稼働している印刷機で直接に測定することはできない。したがって、ここで採るアプローチは応力-強度モデルに依らず、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F (failure probability) を推定する。

>  このように本手法は、厳密な意味で応力-強度モデルに従うものでないことに注意を要する。故障確率 F を強度 R で除した応力 S は参考用である。
...
### 詳細な構造
#### ①未知パラメータから部品負荷の推定
顧客の印刷機の使われ方を推定した<a href="../article1/#未知パラメータ">未知パラメータ</a>をもとに、部品に負荷をかけるための印刷ジョブを生成した。印刷ジョブを生成する都度、未知パラメータの分布からジョブ長と用紙サイズをサンプリングした。
...
#### ②特定の印刷機における部品ライフの推定
印刷機全体の母集団における部品の強度 R をワイブル分布で近似し、生存関数</a> (SF) を推定した。①で作成した印刷ジョブによる搬送距離 ΔT を累積して経年による部品ライフ T1 とする。次のジョブの搬送距離を ΔT として、T2 における条件付き生存率 CS(ΔT|T1) を算出した。
...
部品が生存するか故障するかの判定は、各々の時点における条件付き生存率 (CS) を一様乱数と比較することで行った。部品が故障した場合、あるいは累積ライフが管理目標を超えた場合は新しい部品で置き換え、古い部品のライフと交換した理由を交換履歴に記録した。
...
#### ③応力-強度モデル図作成
このステップは必須ではないが、本スタディが参考とした「応力-強度モデル」を可視化し、理解を助ける意図がある。②で作成した交換履歴をもとに、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求めた。このとき、使用した部品は「故障修理」(故障) によって交換されたものに限り、「予防保守」(打ち切り) で交換された部品は使用しなかった。理由は管理目標の影響を避けるためである。故障確率 F を②で作成した強度 R で除し、応力 S を推定した。
