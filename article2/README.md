...
# 【制作中】2.定期交換部品のライフ推定による交換時期の最適化
この記事は、オンデマンド印刷機の定期交換部品の最適な交換時期をコンピュータ・シミュレーションによって推定するスタディである。

## 目的
デジタル印刷機の保守サービスを最適化するプロセスをコンピュータ上でシミュレーションを行う。この記事はプロセス全体を3つのテーマに分割した 2番目のステップを説明する。最初のステップで推定した顧客による<a href="../article1/">印刷機の使われ方</a>をもとに、定期交換部品を計画的に交換する管理目標が、印刷機の停止時間 (ダウンタイム) と交換される部品数 (コスト) に及ぼす影響を推定し、保守サービスにおける最適な交換時期を推定する。シミュレーションは、プログラミング言語Pythonと無償のシミューレション用パッケージ simpy を用いて構築する。
...
## 部品ライフの推定方法
保守サービスにおける課題は、サービス提供に要するコストと、顧客が受け取るサービス品質との両立にある。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を与えるとした場合、保守サービス提供者の立場で関心があるのは、サービス品質に直結するパラメータである「定期交換部品の交換時期」を変化させたとき、サービス提供に生じるコストへの影響と、顧客に生じるリスクへの影響がある。一つ一つの部品ライフを推定することで、部品の交換数 (コスト) と印刷機の停止時間 (ダウンタイム) を子細に検討できる。

### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)に基づき、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルは応力 S と強度 R が分かっている場合に、故障の確率 F を予測するモデルである。<!-- 応力 S × 強度 R → 故障確率 F という関係を導く。--> 強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。S と R の重なる部分で故障が生じると考え、故障確率 F (failure probability) が与えられる。

<!-- 応力 S > 強度 R の場合に故障が起こると考える -->
...
このとき保守サービスの課題は、顧客の印刷機ごとに異なる応力 S を保守サービスによって調節し、信頼性 F を受け入れ可能となるように機械を運用することと見なせる。顧客先で稼働している印刷機の場合、保守サービスを通じて部品の強度 R を統計的に推定することができる一方、部品にかかる応力 S を測定することはできない。そこで本スタディでは、顧客の使い方を表す「<a href="../article1/#未知パラメータと推定">未知パラメータ</a>」に基づく故障モデルによって応力 S を推定する。

この故障モデルは、未知パラメータから生成した印刷ジョブによる部品負荷 ΔT を累積しながら、部品強度 R に基づく[生存関数](https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0) (survival function, SF) を使用して各時点での生存-故障を判定し、経年にともなう部品の故障をシミュレートする。生存関数は、部品の母集団のうち時刻Tの時点で生存している割合を表し、使用開始時は1で、時間経過とともに0に近づく。時刻T1まで生き残った部品がさらに時刻T2まで生き残る確率は、条件付き生存率 CS(ΔT|T1) = SF(T2)/SF(T1) である。各時点で故障するかどうか、[0-1) の一様乱数によって確率的に判定する。
...
### 部品ライフ推定システム
本システムは、保守サービスの管理目標である「予防保守による交換時期」を信頼性とコストの面から評価するものである。印刷機の定期交換部品のうち、本スタディでは用紙の搬送系の部品 (例: ローラー、ベルト) を取り上げる。次の各機能から全体を構成する。

<dl>
  <dt>①未知パラメータから部品負荷の生成</dt>
  <dd>未知パラメータによって表された印刷機の使われ方をもとに、部品にかかる負荷を生成する。搬送系の部品の負荷は印刷ジョブによって移動する用紙の搬送距離である。ここでは印刷ジョブを無作為に多数作成し、後続の②に出力する。
  </dd>

  <dt>②特定の印刷機における部品ライフの推定</dt>
  <dd>部品強度 R を<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似した。ワイブル分布は修理しない部品ライフのモデル化によく使われ、2つの変数 (形状パラメータ、尺度パラメータ) のみで特徴付けられることから運用にも優れる。これらのパラメータは、保守サービスで交換された部品交換の実績から統計的に算出できる。機能①から入力された印刷ジョブによって部品にかかる負荷を累積し、ワイブル分布にもとづく生存関数 (SF) と一様乱数を用いて生存-故障判定を行い故障発生をシミュレートし、部品ライフを推定した。これと同時に、エンジニアによる予防保守と障害修理をシミュレートし、故障が見込まれるあるいは故障した部品を刷新した。交換された部品のライフ、交換理由、交換に伴う印刷機の停止時間は交換履歴として集められ、後続の③と④に出力する。
  </dd>

  <dt>③応力-強度モデル図作成</dt>
  <dd>機能②で作成した部品の交換履歴を入力して、特定の印刷機における応力 S を対数正規分布で近似し、故障確率 F を推定する。保守サービスの管理目標である「予防保守による交換時期」を段階的に変化させ、上記①～③のステップを繰り返して交換部品数や所要時間を算出する。各段階での応力Sや故障Fの変化を比較することで、適切な管理目標を選択するのに役立つチャートを出力する。
  </dd>

  <dt>④管理目標値の最適値を探索</dt>
  <dd>交換時期の管理目標値ごとのシミュレーション結果を可視化する。機能③と同様に、機能②で作成した部品の交換履歴を入力し、交換時期の管理目標値ごとに算出した「交換部品数」および「交換に伴う機械の停止時間」の変化をグラフィックで出力する。
  </dd>

  <dt>⑤改善効果の予測</dt>
  <dd>交換時期の管理目標値を変更した場合の改善効果を推定する。コスト的な指標として交換部品数を取り上げる。シミュレーション時間の経過に伴う交換部品数をプロットし、管理目標値を変更した場合としなかった場合の差をグラフィックで出力する。これによって、導入効果を定量的に評価し、目標達成の時期を推定する。
  </dd>
...
### 印刷機保守のシミュレーションモデル
デジタル印刷機の部品の故障と交換に関するイベントを表現し、コンピューターでシミュレートするためにシミュレーションモデルを作成した。前述した故障モデルを表現するための、リソース、プロセス、データといったオブジェクトも含む。モデルの主な構成要素を次に示す。
...
### シミュレーション・フレームワーク
コンピュータ上で実行されるシミュレーションでは、1つの環境下に複数のオブジェクトが共存し、互いに同期を取りながら各種イベントが生成し処理されることで全体が進行する。内部の時計が既定の上限に達するまでシミュレーションは継続する。こうした複雑なシミュレーションを簡潔に記述するために、Python言語向けのシミュレーション・フレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を用いてシミュレーションモデルを記述した。シミュレータの概ねの設計を次に示す。ソースコードは付録に添付した。
...
保守サービスにおける定期交換部品の最適な交換時期をシミュレーションで推定した。
...
### 結果
シミュレーション条件を表に示す。この部品の強度は 2パラメータ・ワイブル分布で表した。管理目標は部品強度の尺度パラメータ (1000k) の 0.5～1.5倍の範囲で 0.1 刻みで設定した。

<p align='center'>表. シミュレーション条件</p>

| 意味 | 値 | 補足 |
| --- | --- | --- |
| 部品の強度 R を表すワイブル分布の尺度パラメータ | --designed_life 1000000 | 1000k [A4短辺ページ] と仮定。η値とも呼ばれる |
| (同) 形状パラメータ | --beta 1.8 | 摩耗故障型と仮定。m値とも呼ばれる |
| 予防保守の管理目標(係数) | --wearout_rate 0.5 0.6 0.7 0.8 0.9 1.0 1.1 1.2 1.3 1.4 1.5 | 500k - 1500k まで 100k 刻みで比較する |
| シミュレーション期間 | --maxt 60\*24\*30\*12 | シミュレーション期間を内部時計で 360 日とする |
| シミュレーション回数 | --iter 20 | 20 [回/条件] のシミュレーション結果を平均化する |
| 顧客未知パラメータのトータルエリアカバレッジ | --ac L | トータルエリアカバレッジ(インク被覆率)を中等度とする |
...
#### 停止時間
管理目標を増加させると、「予防保守」(水色) は減る一方で、「障害修理」(桃色) は増加し続ける傾向が見られた (下図)。本シミュレーションでの停止時間は、予防保守が30分に固定、障害修理が60-90分とペナルティを加えた。その結果、トータルの停止時間には大きな変化は見られなかった。
...
#### 印刷機メーカーの立場
「管理目標」を増加させる － 交換時期を遅らせる － と、トータルの部品交換数は低下する。印刷機メーカーの立場では、保守サービスで消費される部品数が減ることは部品コスト面で好ましい。

保守サービスを担当するエンジニアの人的コストは固定費であるため低く抑えたい一方、「障害修理」は優先される保守作業が突発的に発生し、計画的な予防保守を妨げる点は不利となる。

本スタディでは後者を考慮していないが、最適値を求めるためには総合的に判断すべきである。
これについては次のスタディで扱う。

#### 顧客の立場
管理目標を増加させる － 交換時期を遅らせる － と「障害修理」による停止時間が増加する。

障害修理は、修理が終わるまで印刷業者の業務を止める。信頼性が要求される機械では、部品が故障するまで運転を続ける保守運用 (Run-to-Failure) は導入を難しくする。

#### 最適な交換時期

現実には故障は不可避である。部品ライフがワイブル分布に従うと仮定した場合、下図のように、部品の強度 R (青線) の確率密度は幅が広く、低ライフでの初期故障もある程度は起こる。

印刷機の使い方に対応する応力 S (緑色) は保守サービスによって制御可能であり、、、、
