...
#### 交換部品数
管理目標を増加させる、すなわち交換時期を遅らせることに伴い、トータルの部品交換数は低下する傾向であった (下図)。交換理由による内訳は、「予防保守」(水色) は減り続けた一方、「障害故障」(桃色) は増加を続けた。
...
#### 停止時間
管理目標を増加させると、「予防保守」(水色) は減る一方で、「障害故障」(桃色) は増加し続ける傾向が見られた (下図)。本シミュレーションでの停止時間は、予防保守が30分に固定、障害故障が60-90分とペナルティを加えた。その結果、トータルの停止時間には大きな変化は見られなかった。
...
#### 交換部品数
メーカーの立場では、保守サービスで消費される部品数が減ることは部品コスト面で好ましい。一方、エンジニアの人的コストは逆であり、「障害故障」は突発的かつ優先される保守作業が発生し、計画的な予防保守を妨げるために不利となる。本スタディでは後者を考慮していないが最適値を求めるためには総合的する必要がある。これについては次のスタディで扱う。

#### 停止時間
顧客である印刷業者の立場では、「障害故障」(桃色) の発生は、計画外の印刷機の停止を引き起こし、修理が終わるまで業務を止めるために少ない方が望ましい。前述したように、保守サービスを担当するエンジニアにおいても同様である。

#### 最適な交換時期
信頼性が要求される印刷機においては、部品が故障するまで運転を続ける「Run-to-Failure」戦略の受け入れは困難と考えられる。

しかし現実には故障は不可避である。部品ライフがワイブル分布に従うと仮定した場合、下図のように、部品の強度 R (青線) の確率密度は幅が広く、低ライフでの初期故障もある程度は起こる。
