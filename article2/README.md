...
応力-強度モデルで、応力 S × 強度 R → 故障確率 F という関係が導かれる。部品の強度 R は保守サービスを通じて収集することができる。一方、顧客先で稼働する印刷機では、部品にかかる応力 S を測定することはできない。したがってここでのアプローチは、未知パラメータから無作為に推定した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。応力 S は、故障確率 F を強度 R で除して求めた。
