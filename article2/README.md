...
<dl>
  ...
  <dd>前述の故障モデルとシミュレーションモデルを実行して、部品ライフを推定する。
    <dl>
      <dt>故障モデル R</dt>
      <dd>故障モデルの部品強度 R を<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似した。ワイブル分布は修理しない部品ライフのモデル化によく使われ、2つの変数 (形状パラメータ、尺度パラメータ) のみで特徴付けられることから運用にも優れる。各パラメータはコマンドラインで与えた。<!-- 保守サービスで交換された部品交換の実績をもとに統計的に算出することもできる。--></dd>
      <dt>生存-故障判定</dt>
