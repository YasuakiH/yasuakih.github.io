...
#### 停止時間
交換時期を遅らせることによる停止時間は、予防保守と障害修理の合計は標準誤差の範囲内で一定であった。交換理由の別では、「予防保守」(水色) は減少傾向、「障害修理」(桃色) は増加傾向が見られた。本シミュレーションの停止時間は、(計画的な)予防保守が30分なのに対し、(突発的な)障害修理は60-90分とペナルティを加えた。そのため交換部品数の減少は、停止時間の増加で相殺された。
