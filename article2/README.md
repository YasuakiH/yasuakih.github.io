...
#### 印刷機メーカーの視点
管理目標を増加させると (交換時期を遅らせると) 保守サービスで消費される部品交換数は低下し、部品コストの削減が期待できるので好ましいことが分かった。一方、本スタディでは考慮しなかったが、保守サービスを担当するエンジニアの人的コスト (固定費) の観点では計画的な予防保守が好ましく、突発的な保守作業が生じる「障害修理」は、それが高優先であることも含めて不利とと考えられる。最適値の決定には両方面から検討を要するだろう。
...
#### 最適な交換時期
現実には故障は不可避である。部品ライフがワイブル分布に従うと仮定した場合(下図)、部品の強度 R (青線) の確率密度は裾野が広く、低ライフでの故障 (初期故障) も起こりうることがわかった。
...
一方で、印刷機の使い方に対応する応力 S (赤線) は、保守サービスでの交換時期変更によって制御可能であり
