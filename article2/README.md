...
### 保守モデル
デジタル印刷機で行われる、定期交換部品の経年消耗と、エンジニアによる保守サービスをコンピューター・シミュレーションするために「保守モデル」を作成した。保守モデルにはリソース、プロセス、データなどが含まれ、これらが仮想的な環境でそれぞれの役割を果たし、前述した故障モデルを実行する。
...
シミュレーションの間、「環境」内では、印刷ジョブの生成と印刷機への投入、前述した故障モデルに基づく故障の発生、計画的に行われる予防保守、故障した部品を交換する障害修理など、各種イベントが非同期的に発生する。こうした一連の処理を簡潔に記述するため、Python言語のためのフレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用した。
