...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルで、応力 S × 強度 R → 故障確率 F という関係が導かれる。このモデルは材料や部品単体だけでなく、システム全体に適用される場合もある。
