...
<dl>
  ...
  <dd>前述の故障モデルとシミュレーションモデルを実行して、部品ライフを推定する。
    <dl>
      <dt>故障モデル</dt>
      <dd>故障モデルの部品強度 R は<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で表した。ワイブル分布は修理しない部品ライフのモデル化によく使われ、2つの変数 (形状パラメータ、尺度パラメータ) のみで特徴付けられることから運用にも優れる。両パラメータはコマンドラインで与えた。<!-- 保守サービスで交換された部品交換の実績をもとに統計的に算出することもできる。--></dd>
      <dt>生存-故障判定</dt>
      <dd>機能①から入力された印刷ジョブによって与えられた印刷ジョブ長・用紙サイズを基に、部品にかかる負荷を累積して経年を表現する。部品強度 R のワイブル分布にもとづく生存関数 (SF) と一様乱数を用いて生存-故障判定を行い、故障発生をシミュレートした。故障あるいは交換されるまでの累積負荷が部品ライフの推定である。</dd>
      <dt>シミュレーションモデル</dt>
