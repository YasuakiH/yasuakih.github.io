...
| オプション | 意味 | 目的 |
| --- | --- | --- |
| --designed_life 1000000 | 部品ライフ設計値 | 部品ライフ設計値を 1000k [A4短辺ページ] と仮定した。数字に特別な意味はない。 |
| --maxt 60*24*30*12 | シミュレーション期間 | 360 日間を経過したらシミュレーションを終了する。経験的な数値で、10個程度の部品の故障が生じると期待される。 |
| --beta 1.8 | 母集団における部品ライフのワイブル分布形状パラメータ | 摩耗故障型として設定。後続のスタディではこの種類を扱う。 |
| --wearout_rate 0.5 0.6 0.7 0.8 0.9 1.0 1.1 1.2 1.3 1.4 1.5 | 予防保守の管理目標(係数) | 500k - 1500k まで 100k 刻みで行う |
