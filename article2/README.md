...
### 方法
定期交換部品を計画的に交換する「管理目標」が、印刷機の停止時間 (ダウンタイム) や交換される部品数 (コスト) に対し、どのような影響を及ぼすかをシミュレーションによって可視化する。管理目標を一定間隔で変えて停止時間と交換部品数の変化をもとに最適値を検討する。
...
### 結果
#### 交換部品数
交換時期を遅らせることによるトータルの部品交換数は低下傾向であった。交換理由の別では、「予防保守」(水色) は減り続け、一方「障害修理」(桃色) は増え続けた。故障の発生は確率的であり、経年が少ない場合でも故障は起こり、これが障害修理を生じさせる。特に故障がワイブル分布に従うような場合はかなり早期でも故障の可能性がある。シミュレーションでは、交換時期を早めても初期故障を防ぎきれないことを示した。
...
#### 停止時間
交換理由の別では、交換時期を遅らせることで「予防保守」(水色) は減少傾向、「障害修理」(桃色) は増加傾向が見られた。本シミュレーションの停止時間は、(計画的な)予防保守が30分なのに対し、(突発的な)障害修理は60-90分とペナルティを加えた。そのため交換部品数の減少は停止時間の増加で相殺され、予防保守と障害修理を合計した機械の停止時間は、標準誤差の範囲内で一定であった。
...
#### 印刷機メーカーの視点
管理目標を増加させると (交換時期を遅らせると) 保守サービスで消費される部品交換数は低下することから、部品コストの削減が期待でき、好ましい。
