...
## 部品ライフの推定方法
保守サービスにおける課題の一つに、サービス提供に要するコストと、顧客が受け取るサービス品質との両立がある。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を及ぼすと仮定し、交換時期をさまざまに変化させて、架空の印刷ジョブを生成する。これを印刷機へ投入し、部品の摩耗と故障、それに保守サービスによる部品の刷新をシミュレートすることで、部品の交換数 (コスト) と印刷機の停止時間 (ダウンタイム) を予測する。

保守サービスにおける課題の一つに、サービス提供に要するコストと、顧客が受け取るサービス品質との両立がある。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を及ぼす場合、サービス提供者の関心の先は、両者に結びつくパラメータ「定期交換部品の交換時期」の最適化に向けられる。交換時期をさまざまに変化させ、サービスコストへの影響や、突発的な機械停止リスクを予測することが要求される。印刷機に次々と入力される印刷ジョブによって起こる部品の故障、および保守サービスによる部品の刷新をシミュレートし、部品の交換数 (コスト) と印刷機の停止時間 (ダウンタイム) を予測する。

### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)に基づいて、印刷機の「使われ方」から部品ライフを推定する故障モデルを作成した。応力-強度モデルとは、応力 S と強度 R が分かっている場合に、故障の確率 F を予測するモデルである。<!-- 応力 S × 強度 R → 故障確率 F という関係を導く。--> 強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。どちらも特有の形をした分布を持つ。応力 S > 強度 R の場合に故障が起こり、故障確率 F (failure probability) が決まる。
