...
<dl>
  ...
  <dd>印刷機全体の母集団における部品の強度 R を<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似し、<a href="https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0">生存関数</a> (survival function, SF) を推定する。無作為に作成した印刷ジョブによって部品にかかる負荷を累積し、各々の時点における条件付き生存率を計算し、一様乱数との比較によって部品が生存するか故障するかを判定した。故障した部品、あるいは累積ライフが管理目標を超えた部品は新しい部品で置き換え、ライフと理由を交換履歴に記録する。
  </dd>
...
#### ②特定の印刷機における部品ライフの推定
印刷機全体の母集団における部品の強度 R をワイブル分布で近似し、生存関数</a> (survival function, SF) を推定する。無作為に作成した印刷ジョブによって部品にかかる負荷を累積し、各々の時点における条件付き生存率を計算し、一様乱数との比較によって部品が生存するか故障するかを判定した。故障した部品、あるいは累積ライフが管理目標を超えた部品は新しい部品で置き換え、ライフと理由を交換履歴に記録する。
