...
### 応力-強度モデル
本スタディでは、顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を及ぼすと考え、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis) を用いて、機械の使われ方から部品ライフを推定した。このモデルは、材料や部品の強度とそれらにかかる応力の干渉を解析するのに用いられ、本スタディのようにシステム全体にも適用される。 <!--一台一台の印刷機ごとの部品ライフを予測することで、停止時間 (ダウンタイム) と部品交換数 (コスト) を子細に検討できる。-->
...
### 部品の使われ方 - _応力_
最初の記事で、<a href="../article1/">印刷機の使われ方</a> として「未知パラメータ」を推定した。応力-強度モデルにおいて、これらは部品に対する「応力」と見なせる。特に、インク量は画質系の部品ライフに、また用紙の分量やサイズは搬送系の部品ライフに、それぞれ影響する可能性がある。本スタディでは、算出が容易で視覚的に理解しやすい後者、搬送系の部品 (物理的に用紙を輸送する) に着目する。
...
### 部品の強度 - _強度_
部品の強度は、サービスエンジニアによる作業報告や、保守サービスを介して収集される稼働状況から得ることができる。これらの情報には、交換時の部品ライフのほか、交換時の詳細な状況が含まれており、正確な部品強度を
