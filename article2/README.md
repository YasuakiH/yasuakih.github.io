...
### 印刷機保守のシミュレーションモデル
デジタル印刷機の部品の故障と交換に関するイベント (例: 印刷ジョブの生成と印刷機への入力、部品の経年と故障の発生、エンジニアによる部品の交換) を表現し、コンピューターでシミュレートするためにシミュレーションモデルを作成した。前述した故障モデルを表現するための、リソース、プロセス、データといったオブジェクトも含む。主なオブジェクトを次に示す。
...
### シミュレーション・フレームワーク
コンピュータ上で実行されるシミュレーションでは、1つの環境下に複数のオブジェクトが共存し、互いに同期を取りながら各種イベントが生成し処理されることで全体が進行する。こうした複雑なシミュレーションを簡潔に記述するために、Python言語向けのシミュレーション・フレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用して、前述のシミュレーションモデルを記述した。シミュレーションの概ねの設計を次に示す。ソースコードは付録に添付した。
...
### 結果
シミュレーション条件を表に示す。この部品の強度は2パラメータ・ワイブル分布で表され、管理目標はその尺度パラメータ (1000k) の 0.5倍 - 1.5倍 の範囲で 0.1 刻みで設定した。
...
| シミュレーション期間 | --maxt 60\*24\*30\*12 | シミュレーション時刻が 360 日間を経過したらシミュレーションを終了する |
| シミュレーション回数 | --iter 100 | 100 [回/条件] のシミュレーション結果を平均化する |
