...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルは応力 S と強度 R が分かっている場合に、故障の確率 F を予測するモデルである。<!-- 応力 S × 強度 R → 故障確率 F という関係を導く。--> 強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。S と R の重なる部分で故障が生じると考え、その確率が故障確率 F (failure probability) で与えられる。
...
保守サービスの課題は、顧客の印刷機ごとに異なる応力 S を保守サービスによって調節し、信頼性 F を受け入れ可能な水準となるように機械を運用することである。印刷機が顧客先で稼働している場合、部品の強度 R は保守サービスを通じて統計的に推定できる一方、部品にかかる応力 S を直接に測定することは通常できない。そこで本スタディでは、顧客の使い方を表す「未知パラメータ」に基づく故障モデルをもとに、シミュレーションによって応力 S を推定する。

この故障モデルは、未知パラメータから生成した印刷ジョブによる部品負荷 ΔT を累積しながら、部品強度 R に基づく[生存関数](https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0) (survival function, SF) で各時点での生存-故障を判定し、部品の経年と故障をシミュレートする。生存関数は、部品の母集団のうち時刻Tの時点で生存している部品の割合で、使用開始時は1、時間経過とともに0に近づく。時刻T1まで生き残った部品がさらに時刻T2まで生き残る確率は、
条件付き生存率 CS(ΔT|T1) = SF(T2)/SF(T1) によって計算できる。この時に故障する確率は、[0-1) の一様乱数によって確率的に決定することができる。
