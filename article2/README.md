...
## 結論
オンデマンド印刷機の定期交換部品の最適な交換時期を推定するため、応力-強度干渉モデルに基づく故障モデルと、離散事象シミュレーションモデルからなるコンピュータ・シミュレーションを構築した。用紙の搬送系部品を想定して搬送距離を部品にかかる負荷とし、障害修理あるいは予防保守による打ち切りによって交換された部品ライフの分布から応力を推定した。シミュレーションの結果、交換時期の変更が応力に影響を及ぼし、交換部品数 (コスト) と機械停止時間 (ダウンタイム) を変化させることが分かった。また交換時期の変化させて改善効果を推定する応用の可能性も確認した。課題として人的コストを含めたより包括的なシミュレーション実施がある。
