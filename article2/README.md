...
#### ④管理目標値の最適値を探索
このステップでは、管理目標によって交換部品数や所要時間に与える影響をグラフィックで可視化する。上記①～③のステップで得られる交換部品数や所要時間は確率的に変化するため、バラツキを押さえるために所定回数繰り返して算術平均値を用いた。交換時期の管理目標値を範囲内に段階的に設定し、それぞれの管理目標値について交換部品数や所要時間の平均値をグラフィックで可視化した。
