...
* (図の上段) 部品の強度 R (青線) の確率密度は裾野が広く、低ライフでの故障 (初期故障) も起こる可能性があることも示している。
* (図の中段) 障害修理で交換された部品ライフや、予防保守で交換された部品ライフをもとに、対数正規分布で近似した応力 S (赤線) が示されている。これは強度 R とは異なる分布であることもわかる。
* (図の下段) で応力 S が強度 R を上回る領域が強調され、その面積が故障確率に対応している。<br/>
