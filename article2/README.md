...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルでは、応力 S × 強度 R → 故障確率 F という関係を導く。本スタディの場合、部品の強度 R は保守サービスを通じて収集することができるが、その一方、部品にかかる応力 S は、顧客先で稼働している印刷機で直接に測定することはできない。したがって、ここで採るアプローチは応力-強度モデルに依らず、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。

>  このように本手法は、厳密な意味で応力-強度モデルに従っていないことに注意されたい。故障確率 F を強度 R で除して推定した応力 S は参考用である。

### 部品ライフ推定
本スタディは故障現象を理解しやすくするため、印刷用紙の搬送系の部品 (例: ローラー、ベルト) を取り上げ、次のように段階ごとに考える。

<dl>
  <dt>①未知パラメータから部品負荷の推定</dt>
  <dd><a href="../article1/#未知パラメータと推定">未知パラメータ</a>によって表された印刷機の使われ方をもとに、部品にかかる負荷を推定する。搬送系の部品では印刷ジョブによって移動する用紙の搬送距離とする。
  </dd>

  <dt>②特定の印刷機における部品ライフの推定</dt>
  <dd>印刷機全体の母集団における部品ライフを部品強度 R と見なし、これを<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似して<a href="https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0">生存関数</a> (SF) を推定する。無作為に作成した印刷ジョブによって部品にかかる負荷を累積し、各々の時点における条件付き生存率をもとに部品が生存するか故障するかを決定する。部品が故障あるいはその累積ライフが管理目標を超過したら交換する。
  </dd>

  <dt>③応力-強度モデル図作成</dt>
  <dd>交換履歴をもとに、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求める。故障確率 F を強度 R で除し、応力 S を推定する (参考用)。
  </dd>

  <dt>④管理目標値の最適値を探索</dt>
  <dd>交換時期の管理目標値をさまざま変化させ、上記①～③のステップを繰り返して交換部品数や所要時間を算出し、入力に対する出力の変化をグラフィックで可視化する。管理目標の最適値の選択は目視による。
  </dd>

  <dt>⑤信頼性成長を予測</dt>
  <dd>保守サービスによる交換部品数 (コスト) や、印刷機の停止時間 (ダウンタイム) について、管理目標を変更した場合の着地点を予測する。これによって、導入効果を定量的に評価し、目標達成の時期を推定する。ただし、本スタディには、この機能は含まれていない。
  </dd>
...
### 印刷機の保守モデル
デジタル印刷機で行われる、定期交換部品の経年消耗と、エンジニアによる保守サービスをコンピューター・シミュレーションするために「保守モデル」を作成した。前述した故障モデルを実行するために、保守モデルはリソース、プロセス、データといったオブジェクトを含む。

* リソース (プロセスを機能させる有限の資源)
  * 印刷機ユニット
  * 保守エンジニア
* プロセス (所定の条件のもと、リソースを使ってデータを加工・生成する)
  * 印刷
  * 障害修理
* データ (プロセスとプロセスをつなぐ)
  * 印刷ジョブ
...
### フレームワーク
シミュレーションの間、環境内では、印刷ジョブの生成と印刷機への投入、前述した故障モデルに基づく故障の発生、予防保守の計画と実施、故障した部品を交換する障害修理など、各種イベントが非同期的に発生する。こうした一連の処理を簡潔に記述するため、Python言語のためのフレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用した。
...
### 詳細な構造
#### ①未知パラメータから部品負荷の推定
顧客の印刷機の使われ方を推定した<a href="../article1/#未知パラメータ">未知パラメータ</a>をもとに、部品に負荷をかけるための印刷ジョブを生成する。前述のように搬送系の部品に着目することから、部品にかかる負荷は、印刷ジョブによって運動する用紙の搬送距離として見積もった。未知パラメータから「印刷ジョブ長分布」と「用紙サイズ種類」からを無作為にサンプリングしたジョブ長と用紙サイズをもとに、紙送り方向の用紙の長さにジョブ長を乗じた搬送距離を、部品の運動による負荷 ΔT とした。
...
#### ②特定の印刷機における部品ライフの推定
印刷機全体の母集団における部品の強度 R をワイブル分布で近似し、生存関数</a> (SF) を推定する。無作為に多数作成した印刷ジョブによる搬送距離を ΔT を、ジョブごとに累積して経年による部品ライフ累積 T1 とする。次のジョブの累積 T2 による搬送距離を ΔT として T2 = T1 + ΔT で条件付き生存率 CS(ΔT|T1) を算出した。各々の時点における条件付き生存率 (CS) と、一様乱数との比較によって部品が生存するか故障するかを判定した。部品が故障した場合、あるいは累積ライフが管理目標を超えた場合は新しい部品で置き換え、部品ライフと理由を交換履歴に記録する。
...
#### ③応力-強度モデル図作成
交換履歴をもとに、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求める。このとき、使用した部品ライフは「故障」によって交換されたものに限り、予防保守で交換された部品のライフ (打ち切り) は使用しなかった。これは管理目標に基づいているためで、その影響が及ぶことを避けるためである。故障確率 F を強度 R で除し、応力 S を推定した。
...
#### ④管理目標値の最適値を探索
交換時期の管理目標値をさまざま変化させ、上記①～③のステップを繰り返して交換部品数や所要時間を算出し、入力に対する出力の変化をグラフィックで可視化する。管理目標の最適値の選択は目視による。
<div align="center">
  <figure>
...
#### ⑤信頼性成長を予測
保守サービスによる交換部品数 (コスト) や、印刷機の停止時間 (ダウンタイム) について、管理目標を変更した場合の着地点を予測する。これによって、導入効果を定量的に評価し、目標達成の時期を推定する。ただし、本スタディには、この機能は含まれていない。
...
事前設定した打ち切りの管理目標に依存して、故障は影響を受けると考えられる、したがって生の故障データを用いることはできない。
未知パラメータと管理目標をもとに推定した故障分布を用いる。
故障の実績で故障の予測はできない。

前述した「強度」と同様、故障した部品のライフ分布をワイブル分布で表わした場合、
生存関数 SF をで表すことができる。
...
故障した部品の部品ライフをワイブル分布でモデル化し、故障の確率分布を決定した。
