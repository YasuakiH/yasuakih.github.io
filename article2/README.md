...
### 応力-強度モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルでは、応力 S × 強度 R → 故障確率 F という関係が導かれる。部品の強度 R は保守サービスを通じて収集することができる。一方、部品にかかる応力 S は、顧客先で稼働している印刷機で直接に測定することはできない。したがってここでのアプローチは、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。

このように、応力-強度モデルに厳密に従うものでないことに注意されたい。故障確率 F を強度 R で除して応力 S を推定する方法も参考用である。
