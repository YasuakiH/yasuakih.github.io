...
### 結果
シミュレーション条件は表の通りである。この部品の強度は2パラメータ・ワイブル分布で表される。管理目標はその尺度パラメータ (1000k) の 0.5倍 - 1.5倍 の範囲で 0.1刻みで設定した。

<center>表. シミュレーション条件</center>
| 意味 | 目的 | コマンドライン・オプション |
| --- | --- | --- |
| 部品の強度 R | 部品ライフの母集団を表すワイブル分布の尺度パラメータを 1000k [A4短辺ページ] と仮定。 | --designed_life 1000000 |
| 部品ライフ設計値 | 部品ライフの母集団を表すワイブル分布の形状パラメータを摩耗故障型と仮定する。 | --beta 1.8 |
