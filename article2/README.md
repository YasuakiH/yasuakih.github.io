...
#### ②特定の印刷機における部品ライフの推定
印刷機全体の母集団における部品の強度 R をワイブル分布で近似し、生存関数</a> (SF) を推定する。無作為に多数作成した印刷ジョブによって部品にかかる負荷 ΔT を累積し、各々の時点における条件付き生存率を計算し、一様乱数との比較によって部品が生存するか故障するかを判定した。故障した部品、あるいは累積ライフが管理目標を超えた部品は新しい部品で置き換え、ライフと理由を交換履歴に記録する。
