...
<p>
保守サービスの課題は、顧客の印刷機ごとに異なる応力 S に対して、どのように信頼性 F を担保するかである。顧客先で稼働している印刷機の場合、部品の強度 R は保守サービスを通じて統計的に推定することができるが、それに対し、部品にかかる応力 S を直接に測定することはできない。したがって、ここで採るアプローチは応力-強度モデルに依らず、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。
