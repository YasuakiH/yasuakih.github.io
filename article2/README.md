...
部品ライフがワイブル分布に従うと仮定した場合、(下図/上) 部品の強度 R (青線) の確率密度は裾野が広く、低ライフでの故障 (初期故障) も起こりうる。一方、(下図/中) 印刷機の使い方に対応する応力 S (赤線) は、保守サービスでの交換時期変更によって制御可能である。応力-強度干渉モデルでは応力が強度を上回ると故障が起こり、(下図/下) 故障は橙色の領域の面積に対応する。
