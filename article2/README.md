...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルでは応力 S × 強度 R → 故障確率 F という関係が導かれる。強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。SとRの重なる部分で故障が生じ、その確率は故障確率 F (failure probability) である。応力 S は顧客の印刷機ごとに異なるため、保守サービスの課題はどのように折り合いをつけるかということである。
