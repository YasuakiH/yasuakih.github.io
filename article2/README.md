...
保守サービスの課題は、顧客の印刷機ごとに異なる応力 S を保守サービスによって調節し、信頼性 F を受け入れ可能な水準となるように機械を運用することと考えられる。顧客先で稼働している印刷機の場合、保守サービスを通じて部品の強度 R を統計的に推定することができる一方、部品にかかる応力 S を測定することはできない。そこで本スタディでは、顧客の使い方を表す「未知パラメータ」に基づく故障モデルとシミュレーションによって応力 S を推定する。

この故障モデルは、未知パラメータから生成した印刷ジョブによる部品負荷 ΔT を累積しながら、部品強度 R に基づく[生存関数](https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0) (survival function, SF) を使用して各時点での生存-故障を判定し、部品の経年をシミュレートする。生存関数は、部品の母集団のうち時刻Tの時点で生存している部品の割合で、使用開始時は1、時間経過とともに0に近づく。時刻T1まで生き残った部品がさらに時刻T2まで生き残る確率は、条件付き生存率 CS(ΔT|T1) = SF(T2)/SF(T1) である。各時点で故障する確率は、[0-1) の一様乱数によって確率的に判定する。
