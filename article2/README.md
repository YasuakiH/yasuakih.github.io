...
#### 最適な交換時期
(図の上段) 部品の強度 R (青線) の確率密度は裾野が広く、低ライフでの故障 (初期故障) も起こる可能性があることを示す。(図の中段) 障害修理で交換された部品や予防保守で交換 (打ち切り) された部品のライフは、強度 R のワイブル分布から推定した生存関数の影響を受けているものの、対数正規分布で近似した応力 S (赤線) は強度Rとは異なる形状を示した。(図の下段) 応力-強度干渉モデルで応力 S が強度 R を上回る領域 (濃橙色の面積) で故障が起こることが可視化されている。
