...
## 結論
オンデマンド印刷機の定期交換部品の最適な交換時期を推定するため、応力-強度干渉モデルに基づく故障モデルと、離散事象シミュレーションモデルからなるコンピュータ・シミュレーションを構築した。印刷機の使われ方を推定した未知パラメータに基づき、用紙の搬送距離を部品にかかる負荷と見なした。この負荷が累積して部品強度に基づく生存関数から故障交換が生じ、また交換時期に基づく予防保守から打ち切り交換が起こると考え、これらの部品ライフの分布から応力を推定した。交換時期の変更は応力に影響し、交換部品数 (コスト) と機械停止時間 (ダウンタイム) を変化させることが分かった。最後に交換時期の変更による改善効果を推定した。課題として人的コストを含めたより包括的なシミュレーション実施がある。
