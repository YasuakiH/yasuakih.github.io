...
### 部品ライフ推定システム
これは予防保守による交換時期による信頼性とコストへの影響を評価するために設計したソフトウェアである。次の各機能から構成される。

<dl>
  <dt>①未知パラメータから部品負荷の生成</dt>
  <dd>未知パラメータによって表された印刷機の使われ方に基づき、部品にかかる負荷を生成する。搬送系の部品であれば、印刷ジョブによって移動する用紙の搬送距離が負荷となる。この工程では与えられた未知パラメータから印刷ジョブ長の分布と用紙サイズの分布を取得し、それぞれからサンプリングした印刷ジョブ長と用紙サイズを含む印刷ジョブを生成し、後続の②に出力する。</dd>
...
| 顧客未知パラメータのトータルエリアカバレッジ | --ac L | トータルエリアカバレッジ(インク被覆率)を低(Low)とする(この指定は無視される) |
