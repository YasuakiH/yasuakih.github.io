...
#### 交換部品数
管理目標を増加させる、すなわち交換時期を遅らせることに伴い、トータルの部品交換数は低下した (下図)。交換理由の内訳は、「予防保守」(水色) が次第に減る一方、「障害故障」(桃色) が増加し続けた。
...
#### 停止時間
管理目標を増加させることで「予防保守」(水色) が次第に減り、「障害故障」(桃色) が増加する傾向が見られる。本シミュレーションで設定した停止時間は、予防保守が30分に固定した一方、障害故障が60-90分とペナルティが加わることで、トータルの停止時間には大きな変化は見られなかった。
