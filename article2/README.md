...
#### ③応力-強度モデル図作成
このステップは必須ではないが、本スタディが参考とした「応力-強度モデル」を可視化する狙いがある。②で作成した交換履歴をもとに、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求めた。このとき、使用した部品は「故障修理」(故障) によって交換されたものに限り、管理目標に基づいて「予防保守」(打ち切り) で交換された部品は使用しなかった。故障確率 F を強度 R で除し、応力 S を推定した。
