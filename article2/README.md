...
### 結果
シミュレーション条件は次の通りである。管理目標は部品ライフ設計値 (1000k) の 0.5倍 - 1.5倍 の範囲で0.1刻みで設定した。この架空の設計値は、部品の強度 R を与えるものである。

| 意味 | 目的 | コマンドライン・オプション |
| --- | --- | --- |
| 部品ライフ設計値<br/>ワイブル分布の尺度パラメータ | 部品ライフ設計値を 1000k [A4短辺ページ] と仮定。部品ライフの母集団におけるワイブル分布の尺度パラメータ。 | --designed_life 1000000 |
| 部品ライフ設計値 | 部品ライフを摩耗故障型として設定。部品ライフの母集団におけるワイブル分布形状パラメータ。 | --beta 1.8 |
