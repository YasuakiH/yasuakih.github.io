...
本スタディでは、故障分布の推定として理解しやすい印刷用紙の搬送系の部品 (例: ドラム、ベルト、ローラー) で考える。この場合、部品が運動する回転数や搬送距離として負荷を見積もることができる。次のように段階的にこの課題に対応した。
