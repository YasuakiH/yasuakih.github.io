...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルでは、応力 S × 強度 R → 故障確率 F という関係を導く。本スタディの場合、部品の強度 R は保守サービスを通じて収集することができるが、その一方、部品にかかる応力 S は、顧客先で稼働している印刷機で直接に測定することはできない。したがって、ここで採ったアプローチは応力-強度モデルに依らず、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。

>  このように本手法は、厳密な意味で応力-強度モデルに従っていないことに注意されたい。故障確率 F を強度 R で除して推定した応力 S は参考用である。

### 部品ライフ推定
本スタディでは、故障現象を理解しやすくするため、印刷用紙の搬送系の部品 (例: ローラー、ベルト) で考える。次のように段階的に対応する。

<dl>
  <dt>①未知パラメータから部品負荷の推定</dt>
  <dd><a href="../article1/#未知パラメータと推定">未知パラメータ</a>によって表された印刷機の使われ方を元に、部品にかかる負荷を推定する。搬送系の部品の場合、印刷ジョブによって移動する用紙の搬送距離として見積もる。
  </dd>

  <dt>②特定の印刷機における部品ライフの推定</dt>
  <dd>印刷機全体の母集団における部品の強度 R を<a href="https://ja.wikipedia.org/wiki/%E3%83%AF%E3%82%A4%E3%83%96%E3%83%AB%E5%88%86%E5%B8%83">ワイブル分布</a>で近似し、これによって<a href="https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0">生存関数</a> (SF) を推定する。無作為に作成した印刷ジョブによって部品にかかる負荷を累積し、各々の時点における条件付き生存率から部品が生存するか故障するかを判定する。部品は故障あるいは累積ライフが管理目標を超えた時点で交換し、履歴に記録する。
  </dd>

  <dt>③応力-強度モデル図作成</dt>
  <dd>交換履歴を元に、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求める。故障確率 F を強度 R で除し、応力 S を推定する。
  </dd>
...
### 詳細な構造
#### ①未知パラメータから部品負荷の推定
顧客の印刷機の使われ方を推定した<a href="../article1/#未知パラメータ">未知パラメータ</a>をもとに、部品に負荷をかけるための印刷ジョブを生成する。前述のように搬送系の部品に着目することから、部品にかかる負荷は、印刷ジョブによって運動する用紙の搬送距離として見積もった。未知パラメータから「印刷ジョブ長分布」と「用紙サイズ種類」からを無作為にサンプリングしたジョブ長と用紙サイズを元に、紙送り方向の用紙の長さにジョブ長を乗じた搬送距離を、部品の運動による負荷 ΔT とした。
...
#### ②特定の印刷機における部品ライフの推定
印刷機全体の母集団における部品の強度 R をワイブル分布で近似し、生存関数</a> (SF) を推定する。無作為に多数作成した印刷ジョブによる搬送距離を ΔT を、ジョブごとに累積して部品の経年によるライフ累積 T1 とする。次のジョブの累積 T2 による搬送距離を ΔT として T2 = T1 + ΔT で条件付き生存率 CS(ΔT|T1) を算出した。各々の時点における条件付き生存率 (CS) と、一様乱数との比較によって部品が生存するか故障するかを判定した。部品が故障した場合、あるいは累積ライフが管理目標を超えた場合は新しい部品で置き換え、部品ライフと理由を交換履歴に記録する。
...
#### ③応力-強度モデル図作成
交換履歴を元に、特定の印刷機における部品ライフをワイブル分布で近似し、故障確率 F を求める。このとき、使用した部品ライフは「故障」によって交換されたものに限り、予防保守で交換された部品のライフ (打ち切り) は使用しなかった。これは管理目標に基づいているためで、その影響が及ぶことを避けるためである。故障確率 F を強度 R で除し、応力 S を推定した。
