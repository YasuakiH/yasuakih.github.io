...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルでは応力 S × 強度 R → 故障確率 F という関係が導かれる。強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。SとRの重畳する部分で故障が生じ、その確率が故障確率 F (failure probability) である。応力 S は顧客の印刷機ごとに異なるため、保守サービスの課題はどのように折り合いをつけるかということである。

<div align="center">
  <figure>
    <img src="img/故障モデル-応力強度モデルの概説.png"/>
	<br/>
    <figcaption>図. 応力-強度モデルによる故障発生。(左) 応力Sと強度Rの分布が離れていれば故障は起こりにくい。(右) これらの分布が重なるにつれて故障が起こりやすくなる。</figcaption>
  </figure>
</div>

<p>

顧客先で稼働している印刷機の場合、部品の強度 R は保守サービスを通じて統計的に推定することができるが、それに対し、部品にかかる応力 S を直接に測定することはできない。したがって、ここで採るアプローチは応力-強度モデルに依らず、未知パラメータから無作為に作成した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。
