...
## 部品ライフの推定方法
保守サービスにおける課題の一つに、サービス提供に要するコストと、顧客が受け取るサービス品質との両立がある。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を及ぼすと仮定した場合、サービス提供者の関心は、両者に直結するパラメータ「定期交換部品の交換時期」を最適にすることに向けられる。交換時期パラメータを変化させたとき、サービスコストへの影響と、突発的な機械停止リスクへの影響を予測する必要がある。印刷機に次々と入力される印刷ジョブに対する部品の生存-故障判定と、保守サービスによる部品の交換をシミュレートすることで、部品の交換数 (コスト) と印刷機の停止時間 (ダウンタイム) を予測できる。
