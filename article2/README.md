...
### フレームワーク
シミュレーションの間、環境内では、印刷ジョブの生成と印刷機への投入、故障モデルに基づく故障の発生、予防保守の計画と実施、故障した部品を交換する障害修理など、各種イベントが非同期的に発生する。こうした一連の処理を簡潔に記述するため、Python言語のためのフレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用した。
