...
### 故障モデル
本スタディでは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)を参考に、機械の使われ方から部品ライフを推定する故障モデルを作成した。応力-強度モデルでは応力 S × 強度 R → 故障確率 F という関係が導かれる。強度 R (resistance) は設計や製造で決まり、応力 S (stress) は使い方で決まる。SとRの重なる部分で故障が生じると考え、その確率を故障確率 F (failure probability) という。
...
保守サービスの課題は、顧客の印刷機ごとに異なる応力 S に対して、どのように信頼性 F を推定するかである。顧客先で稼働している印刷機の場合、部品の強度 R は保守サービスを通じて統計的に推定することができるが、それに対し、部品にかかる応力 S を直接に測定することはできない。そのため、本スタディは応力-強度モデルで故障確率 F を推定するものではなく、顧客の使い方を表す「未知パラメータ」を用いたシミュレーションによって故障確率 F を推定するアプローチを考案した。

考案した故障モデルは、未知パラメータから生成した印刷ジョブによる部品負荷を累積しながら、部品強度 R に基づく[生存関数](https://ja.wikipedia.org/wiki/%E7%94%9F%E5%AD%98%E9%96%A2%E6%95%B0) (survival function, SF) でその時点での生存-故障を判定し、部品の経年に伴う故障をシミュレートするものである。
...
このように本手法は、厳密な意味で応力-強度モデルに従うものでなく、後述した故障確率 F を強度 R で除した応力 S は参考用である。

### 部品ライフ推定
本スタディは故障現象を理解しやすくするため、印刷用紙の搬送系の部品 (例: ローラー、ベルト) を取り上げ、次の5つの機能ブロックを組み合わせて部品ライフを推定し評価する。

<dl>
  <dt>①未知パラメータから部品負荷の推定</dt>
  <dd><a href="../article1/#未知パラメータと推定">未知パラメータ</a>によって表された印刷機の使われ方をもとに、部品にかかる負荷を推定する。搬送系の部品では、印刷ジョブによって移動する用紙の搬送距離が一例である。
  </dd>
...
### 印刷機の保守モデル
デジタル印刷機で行われる、定期交換部品の経年消耗と、エンジニアによる保守サービスをコンピューターでシミュレートするために「保守モデル」を作成した。保守モデルは前述した故障モデルを実行するために、リソース、プロセス、データといったオブジェクトを含む。
...
### フレームワーク
シミュレーションの間、環境内では、印刷ジョブの生成と印刷機への入力、故障の発生、予防保守の計画と実施、故障した部品の交換など、各種イベントが非同期的に発生する。こうした一連の処理を簡潔に記述するため、Python言語のためのフレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用した。
