...
## 部品ライフの推定方法
保守サービスにおける課題の一つに、サービス提供に要するコストと、顧客が受け取るサービス品質との両立がある。顧客による<a href="../article1/">印刷機の使われ方</a>が部品ライフに影響を及ぼす場合、サービス提供者の関心の先は、両者に結びつくパラメータ「定期交換部品の交換時期」の最適化に向けられる。交換時期を変化させ、サービスコストへの影響や、突発的な機械停止リスクへの影響を予測することが要求される。印刷機に次々と入力される印刷ジョブによって起こる部品の故障、そして保守サービスによる部品の刷新をシミュレートすれば、部品の交換数 (コスト) と印刷機の停止時間 (ダウンタイム) を予測できる。
