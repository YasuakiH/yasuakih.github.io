...
## 目的
デジタル印刷機の保守サービスを最適化するプロセスをコンピュータ上でシミュレーションを行う。この記事はプロセス全体を3つのテーマに分割した 2番目初のステップを説明する。最初の記事で推定した<a href="../article1/">印刷機の使われ方</a>をもとに、計画的に定期交換部品を交換する管理目標が、印刷機の停止時間 (ダウンタイム) と交換される部品数 (コスト) に与える影響を推定し、保守サービスによる最適な交換時期を決定する。汎用プログラミング言語のPythonとシミューレション用パッケージ simpy でシミュレーションを構築する。
