...
本スタディで参考にした故障モデルは、確率的に故障が発生する[応力-強度モデル](https://en.wikipedia.org/wiki/Stress%E2%80%93strength_analysis)で、機械の使われ方から部品ライフを推定するものである。このモデルは一般に、材料や部品の強度とそれらにかかる応力との干渉を解析したり、ときにはシステム全体に適用される。部品ライフを推定すれば、部品の交換数 (コスト) と、印刷機の停止時間 (ダウンタイム) を子細に検討できる。

応力-強度モデルには、応力 S × 強度 R → 故障確率 F という関係がある。部品の強度 R は保守サービスを通じて収集することができる。一方、顧客先で稼働する印刷機では、部品にかかる応力 S を測定することはできない。したがってここでのアプローチは、未知パラメータから無作為に推定した部品負荷を累積しながら、部品強度 R に基づく生存関数 SF で生存-故障を判定するシミュレーションによって故障確率 F を推定する。応力 S は、故障確率 F を強度 R で除して求めた。

このスタディでは次のように段階的にこの課題に対応した。
