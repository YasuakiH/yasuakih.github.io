...
### フレームワーク
シミュレーションの間、環境内では各種イベントが平行して進行する。こうした複雑な処理を簡潔に記述するために、Python言語のためのシミュレーション・フレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用した。これによってコーディングは課題解決の本質に注力することができた。
