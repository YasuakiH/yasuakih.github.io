...
### シミュレーション・フレームワーク
コンピュータ上で実行されるシミュレーションでは、1つの環境下に複数のオブジェクトが共存し、互いに同期を取りながら各種イベントが生成し処理されることで全体が進行する。シミュレーションは、内部の時計が上限に達するまで継続する。

こうした複雑なシミュレーションを簡潔に記述するために、Python言語向けのシミュレーション・フレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用して、前述のシミュレーションモデルを記述した。シミュレーションの概ねの設計を次に示す。ソースコードは付録に添付した。
