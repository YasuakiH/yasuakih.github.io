...
### 印刷機保守のシミュレーションモデル
デジタル印刷機に関連するさまざまな事象を表現し、定期交換部品の経年と故障、エンジニアによる保守サービスをコンピューターでシミュレートするために「シミュレーションモデル」を作成した。シミュレーションモデルは前述した故障モデルを実行するために、リソース、プロセス、データといったオブジェクトを含む。以下は主なものである。

* リソース (プロセスを機能させる有限の資源)
  * 印刷機ユニット
  * 保守エンジニア - 予防保守、障害修理を担当する
* プロセス (所定の条件のもと、リソースを使ってデータを加工・生成する)
  * 印刷 (内部に、生存-故障判断、部品経年加算を含む)
  * 予防保守 (印刷機ユニットと部品を確保し、短時間で交換する)
  * 障害修理 (予防保守と同様だが長時間を要する)
* データ (プロセスとプロセスをつなぐメッセージ)
  * 印刷ジョブ (印刷ジョブ長、用紙サイズなど保持する)
  * 部品 (部品ライフを保持する)
  * ログ (交換履歴、部品ライフ、印刷ジョブなどを保持する)
