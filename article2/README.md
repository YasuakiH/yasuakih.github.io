...
保守サービスの課題は、顧客の印刷機ごとに異なる応力 S を保守サービスによって調節し、信頼性 F を受け入れ可能な水準となるように機械を運用することと考えられる。印刷機が顧客先で稼働している場合、部品の強度 R は保守サービスを通じて統計的に推定することができるが、部品にかかる応力 S を直接に測定することは困難である。そこで本スタディでは、顧客の使い方を表す「未知パラメータ」に基づく故障モデルをもとに、シミュレーションによって応力 S を推定する。
...
条件付き生存率 CS(ΔT|T1) = SF(T2)/SF(T1) である。各時点で故障する確率は、[0-1) の一様乱数によって確率的に判定する。
