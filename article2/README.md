...
<blockquote>
<div align="center"><figure><img src="img/応力強度モデルチャート(AC=L,PL=M)(iter=20)/応力-強度干渉グラフ(AC=L,PL=M)(0.80).png"/><br/><figcaption>応力-強度干渉モデル図を示す (AC=L,PL=M)(管理目標係数0.80)<br/>(図の上段) 部品の強度 R (青線) の確率密度は裾野が広く、低ライフでの故障 (初期故障) も起こる可能性があることも示している。(図の中段) 障害修理で交換された部品ライフや、予防保守で交換された部品ライフをもとに、対数正規分布で近似した応力 S (赤線) が示されている。これは強度 R とは異なる分布であることもわかる。(図の下段) で応力 S が強度 R を上回る領域が強調され、その面積が故障確率に対応している。
