...
#### 交換部品数
メーカーの立場では、保守サービスで消費される部品数が減ることはコスト面で好ましい。一方、エンジニアの人的コストの観点では「障害故障」は突発的かつ優先的な保守作業が発生し、計画的な予防保守を妨げることから不利となる。本シミュレーションでは後者を考慮していないが、最適値を求めるためには考慮を要する。

#### 停止時間
顧客である印刷業者の立場では、「障害故障」(桃色) の発生は、計画外の印刷機の停止を引き起こし、修理が終わるまで業務を止めるために少ない方が望ましいのは言うまでもない。前述したように、保守サービスを担当するエンジニアにおいても同様である。
