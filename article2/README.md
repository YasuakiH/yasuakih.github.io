...
### 保守モデル
保守モデルは、デジタル印刷機における定期交換部品の経年消耗と、エンジニアによる保守サービスをシミュレートするとともに、部品ライフと交換理由を履歴に記録する機能を持つ。加えて、印刷機に入力される印刷ジョブの生成、前述した故障モデルに基づく故障の発生、計画的に行われる予防保守、故障した部品を交換する障害修理など、各種イベントが非同期的に発生する。こうした一連の処理を記述する保守モデルを作成した。
