...
シミュレーション中の環境では、印刷機に入力される印刷ジョブの生成、前述した故障モデルに基づく故障の発生、計画的に行われる予防保守、故障した部品を交換する障害修理など、各種イベントが非同期的に発生する。こうした一連の処理を簡潔に記述するため、Python言語のためのフレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用した。
