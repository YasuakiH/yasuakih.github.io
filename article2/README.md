...
### シミュレーション・フレームワーク
シミュレーションモデルの環境内では複数のオブジェクトが共存し、互いに同期を取りながら各種のイベントが生成・処理される。こうした複雑なシミュレーションを簡潔に記述するために、Python言語向けのシミュレーション・フレームワーク [Simpy](https://simpy.readthedocs.io/en/latest/) を使用し、前述のシミュレーションモデルを記述した。
