...
保守サービスの課題は、顧客の印刷機ごとに異なる応力 S に対して、どのように信頼性 F を推定するかとなる。顧客先で稼働している印刷機の場合、部品の強度 R は保守サービスを通じて統計的に推定することができても、部品にかかる応力 S を直接に測定することはできない。そのため、本スタディは応力-強度モデルで故障確率 F を推定するものではなく、顧客の使い方を表す「未知パラメータ」を用いたシミュレーションによって故障確率 F を推定するアプローチを考案した。
