...
### 全体の構造
設計はトップダウンである。プログラミング言語はPythonを用いた。シミュレーションは直列に実行する単純なものであり、シミュレーション用のフレームワークも使用しない。処理時間を短縮するため multiprocessing モジュールを用いた並列処理を加えたが、結果に違いはない。次は全体の構造である (丸括弧内は関数名)。
