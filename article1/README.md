...
<figure>
  ...
  <figcaption>Step1 未知パラメータ (潜在的な母集団を表すパラメータ) を無作為に設定し、Step2 サンプリングした印刷ジョブから資源の消費量を計算し、その累積値が一定の上限に達した時点で止める。Step3 結果を事実と照らして妥当かどうかを判断する。モンテカルロ法ではこれらのステップを多数繰り返し、妥当と見なした結果の集まりから未知パラメータに対する知見を得る。</figcaption>
</figure>
