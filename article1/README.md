...
#### 評価0: インク量 (累積)
総インク量は印刷ジョブごとの消費インク量を累積して算出した。この量はシミュレーションを終了する条件として用いているため、期待値と推定値はほぼ一致する。
...
#### 評価1: 用紙サイズ別の印刷枚数 [枚] 
期待値とシミュレーション値との比較は、用紙サイズ別の分布として交差エントロピーにより数値化して行った。

<div align="center">
<figure>
  <img src="img/妥当性評価-資源消費量-印刷用紙.png"/>
  <br/>
  <figcaption>図. 印刷用紙の消費量の期待値 (左=灰色)と、シミュレーション値 (右=色別) を比較した。X軸の用紙サイズ別にY軸の印刷枚数の累積値を表示している。左端にそれぞれの総計と比率を示した。図の例では推定/期待の比率は0.90で10%の誤差がある。</figcaption>
</figure>
</div>

#### 評価2: ページ数合計
用紙全体としての収支を評価できるよう、印刷枚数の総計値を比較した。上図の左端に例を示す。
