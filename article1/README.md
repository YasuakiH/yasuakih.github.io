...
## 目的
デジタル印刷機の保守サービスを最適化するプロセスをコンピュータ上でシミュレーションを行う。この記事はプロセス全体を3つのテーマに分割した最初のステップを説明する。汎用のプログラミング言語Pythonとパッケージでシミュレーションを構築し、架空の印刷機の未知パラメータを推定する。
...
### 印刷機セグメントの推定
印刷機セグメントマップ上で、計4件の OK 結果は図のように対応する。75%はトータルエリアカバレッジ=M(Mid)、印刷ジョブ長=L(Low)に集まった。この結果から、この架空の印刷機は、比較的中等のエリアカバレッジで、比較的少数部数の出力を行っていると考えることができる。
...
## 結論
保守サービスを最適化を想定した「印刷機の使われ方」の探索をモンテカルロ法シミュレーションで行った。ブラックボックスである「印刷機の使われ方」を、3つの未知パラメータとして表現し、遠隔保守サービスで得られる統計的な情報から、これらの未知パラメータの「もっともらしい」値を推定することができた。課題として、実データによる検証や調整と、機能面ではカラー印刷への対応がある。
