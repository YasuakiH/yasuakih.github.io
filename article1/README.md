...
## 未知パラメータと推定
### 部品ライフ管理の最適化と未知パラメータ
印刷機にかかる負荷によって部品ライフは左右される。部品の保守管理を最適化するために、顧客がどのように印刷機を使用し、どんな負荷が部品にかかるかを知る必要がある。前述のように印刷ジョブは外部から知ることができないため、入手できる情報をもとに部品ライフを表す別の指標を策定する。こうした印刷業者の使い方を特徴づける情報は、外部から観測できないことから「未知パラメータ」と呼ぶ。

<!--ただし、印刷業者が受注した印刷物の仕様 (例: 用途、寸法、用紙銘柄、色指定、加工) や、印刷業者が顧客から預かった印刷原稿自体は未知パラメータに含めない。また、-->
顧客が不良とみなす画質不具合の種類や程度などの品質基準も関心の対象となりうるが、知ることはできないので、個々の印刷機の部品ライフに (暗黙に) 含まれると考える。<!--特に考慮はしない。-->
...
#### シミュレーション終了
印刷ジョブを架空の印刷機に入力して消費される資源量を見積もり、保守サービスで把握された稼働実績と比較する。両者の違いが無視できるなら、仮定した未知パラメータを「もっともらしい」と考える。
