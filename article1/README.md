...
<figure>
  ...
  <br/>
  <figcaption>図5. 用紙サイズ別のジョブ比率。示した例では、葉書を除く 4種類が利用可能で、A3および長尺のジョブ数が大半を占めている。A3と長尺でジョブ数はほぼ同じだが、印刷ジョブ長の分布に応じて消費される用紙枚数は異なる。</figcaption>
</figure>
</div>
...
### インク量の算出
生成した印刷ジョブから、前述の「資源量の算出」に示した式を用いてインク量を算出した。
...
<figure>
  ...
  <figcaption>図5. モデルの実現性スタディ 収支計算</figcaption>
</figure>
