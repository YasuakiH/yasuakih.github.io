...
### 印刷機セグメントの推定
印刷機セグメントマップ上で、計4件の OK 結果は図のように対応した。75%はトータルエリアカバレッジ=M(Mid)、印刷ジョブ長=L(Low)に集まった。残り25%は別のセグメントであったが、印刷ジョブ長=M(Mid)は異なるものの隣接し、エリアカバレッジは同じであった。この結果から、この架空の印刷機は、比較的中等のエリアカバレッジで、比較的少数部数の出力を行っていると見なせる。
