...
部品にかかる負荷は印刷ジョブによって発生するが、印刷ジョブは1つ1つが異なっており、外部から観察できない。したがって、印刷ジョブを生成する要因への理解が必要となり、これが未知パラメータとなる。このスタディでは、顧客の印刷ジョブを生成する要因として次の項目に着目する。
...
### モンテカルロ法による未知パラメータ推定
印刷機は複雑なシステムのため、未知パラメータを推定する数式を作り、解析的に解くことは困難を伴うと予想される。本スタディではコンピューターを用いたモンテカルロ法によって解を探索する。
